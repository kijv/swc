...
impl Diagnostic {
    ...
    pub fn message(&self) -> Cow<'static, str> {
        match &self.inner.1 {
            ...
            DiagnosticKind::DuplicateLinkDefinition(label) => {
                format!("Duplicate link reference definition '{label}' (first definition used)",)
                    .into()
            }
            DiagnosticKind::UnresolvedLinkReference(label) => {
                format!("Unresolved link reference '{label}' (rendered as literal text)",).into()
            }
